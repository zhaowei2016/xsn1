  ...
  <div class="home">
    ...
    <div class="content">
      <div class="bgw content_t">
        ...
        <span>2022年秋季学生饮用奶征订工作已经开始</span>
      </div>
      ...
      <van-field v-model="schoolName" is-link readonly name="picker1" placeholder="请选择学校" @click="pick1 = true" />
...
import { getSchoolList, getSchoolCList, getProductList,getArea } from '@/api/api'
...
export default {
  ...
  setup() {
    ...
    onBeforeMount(async () => {
      ...
      state.columns1 = state.columns1.concat(res)
      localStorage.removeItem('submitObj')
    })
    const onConfirm1 = async (value: any) => {
      state.schoolName = value.schoolName
      state.schoolId = value.schoolId
      state.pick1 = false
      //年级班级
      state.address = value.province + value.city + value.district
